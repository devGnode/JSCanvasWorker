var graphicalUserInterface = function( setting, _self ){
	...
	self.resetScreen = function( color ){
		ctxMonitor.fillStyle = color;
		ctxMonitor.fillRect( 0,0, screen_x, screen_y );
	};
	// resize Monitor
	self.resize = function( x, y ){
		self.screen_x = screen_x = x;
		self.screen_y = screen_y = y;
	...
	["get","set"].map( function( val ){
		...
		self[ val+"RawPixel" ] = function( offset, color ){
			if( val == "set" ){
				screen_d.data[ offset+0x00 ] = ( color >> 0x10 )&0xff;
				screen_d.data[ offset+0x01 ] = ( color >> 0x08 )&0xff;
				screen_d.data[ offset+0x02 ] = ( color )&0xff;
				screen_d.data[ offset+0x03 ] = 255;
			}else{
	...
	var intToRgb = self.intToRgb = function( color ){
